...
```shell
./eth-cli account <your-address> --chain-id ${Name-Chain}-Tigris --node https://dataseed5.${domainchain}:443 --indent --trust-node
```
...
There are two ways of creating an account on ${Name Chain}: creating a key in a web wallet and creating a key via `eth-cli`. Please make sure you backup your mnemonic.

### Web Wallet

+ You can create a key via [web wallet](https://testnet-explorer.${domainchain}/create)
...
+ You can restore your key [here](https://testnet-explorer.${domainchain}/unlock)
...
```bash
$  ./eth-cli account bnc1wwgakqy32m7vdnlf00pctf9hnaak37eh7wkmqa --trust-node --chain-id ${Name-Chain}-Tigris --node https://dataseed5.${domainchain}:443
```
Example on **testnet**:

```bash
$  ./eth-cli account bnc1wwgakqy32m7vdnlf00pctf9hnaak37eh7wkmqa --trust-node --chain-id=${Name-Chain}-Ganges --node=data-seed-pre-2-s1.${domainchain}:80
{
...
You can get test ${SYMBOL} by following [this guide](https://www.${name}.vision/tutorials/${name}-dex-funding-your-testnet-account).
