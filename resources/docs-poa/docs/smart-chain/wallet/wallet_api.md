${Name Chain} Wallet injects a global API into websites visited by its users at `window.${NameChain}`.

This API specification borrows heavily from API MetaMask provided, considering the massive adoption. So Web3 site developers can easily connect their dApps with the ${Name Chain} Wallet. The APIs allow websites to request users' ${Name} Smart Chain addresses, read data from the blockchain the user is connected to, and prompt the users to sign messages and transactions.

The presence of the provider object `window.${NameChain}` indicates a ${Name Chain}/${Name} Smart Chain user.
...
Currently (version 1.112.8) as ${Name Chain} Wallet natively supports ${Name Chain}, we are planning to open a series of APIs for dApp developers to interact with ${Name Chain}. At the end of the day, most [APIs available in ${Name Chain} javascript sdk](https://github.com/${githubusername}/javascript-sdk/tree/master/docs) would be available.

Currently, only the following is supported:
* [`transfer`](https://github.com/${githubusername}/javascript-sdk/tree/master/docs#transfer-tokens) 

## Difference with MetaMask

!!! warning

    Please read through this section if you are a web3 developer who has integrated with MetaMask and interested in integrating with ${Name Chain} Wallet.

### Inpage injected object

The biggest difference between ${Name Chain} Wallet and MetaMask is we inject `${NameChain}` rather than `ethereum` (or `web3`) to the web page. So user could keep two extensions at the same time.

### ${NameChain}.request({method: "eth_sign", params: ["address", "message"])
...
### ${NameChain}.request({method: "eth_accounts"})
...
!!! warning
    ...
    In this implementation, some APIs will be supported as [Legacy API](#legacy-api) (For example we will still implement the `chainIdChanged` on ${NameChain} object until MetaMask formally deprecates it).

## Basic Usage

For any non-trivial ${Name Chain} web application — a.k.a. web3 site — to work, you will have to:

1. Detect the ${Name Chain} provider (`window.${NameChain}`)
2. Detect which ${Name Chain} network the user is connected to
3. Get the user's ${Name Chain} account(s)
...
We made a tiny lib [${name}-connector](https://www.npmjs.com/package/@${name}-chain/${name}-connector) that implements the [AbstractConnector](https://github.com/NoahZinsmeister/web3-react/blob/v6/packages/abstract-connector/src/index.ts#L4) interface of [web3-react](https://github.com/NoahZinsmeister/web3-react) library. You can add this to your project in parallel with [injected-connector](https://github.com/NoahZinsmeister/web3-react/tree/v6/packages/injected-connector) by: `yarn add @${name}-chain/${name}-connector` or `npm i @${name}-chain/${name}-connector`.

```
import { ${Name}Connector } from '@${name}-chain/${name}-connector'

export const ${name} = new ${Name}Connector({
  supportedChainIds: [${MainnetId}, ${TestnetId}] // later on 1 ethereum mainnet and 3 ethereum ropsten will be supported
})

// invoke method on ${name} e.g.
await ${name}.activate();
await ${name}.getAccount();
await ${name}.getChainId();
```

* use-wallet

There is an [example](https://github.com/aragon/use-wallet/tree/master/examples/${name}-chain) in use-wallet origin repo shows how to 'inject' a customized web3-react connector to `UseWalletProvider`:
...
render(
  <UseWalletProvider
    connectors={{
      ${name}: {
        web3ReactConnector() {
          return new ${Name}Connector({ supportedChainIds: [${MainnetId}, ${TestnetId}] })
        },
...
!!! warning
    ...
    At the moment, the [`${NameChain}.chainId`](#ethereum-chainid) property and the [`chainChanged`](#chainchanged) event should be preferred over the `eth_chainId` RPC method.
...
These are the IDs of the ${Name} Chains that ${Name Chain} Wallet supports by default.

| Hex  | Decimal | Network                                        |
| ---- | ------- | ---------------------------------------------- |
| ${HexMainnetId} | ${MainnetId}      | ${Name} Smart Chain Main Network (${name}-mainnet) |
| ${HexTestnetId} | ${TestnetId}      | ${Name} Smart Chain Test Network (${name}-testnet) |

This API can also return chain ids of ${Name} Chains if users switch to them. The possible return value would be:
| Chain Id             | Network                                  |
| -------------------- | ---------------------------------------- |
| ${Name-Chain}-Tigris | ${Name Chain} Main Network (bbc-mainnet) |
| ${Name-Chain}-Ganges | ${Name Chain} Test Network (bbc-testnet) |
...
**NOTE:** See the [Chain IDs section](#chain-ids) for important information about the ${Name Chain} Wallet provider's chain IDs.


A hexadecimal string representing the current chain ID.

### ${NameChain}.autoRefreshOnNetworkChange
...
```javascript
${NameChain}.autoRefreshOnNetworkChange = false;
```

## Methods

### ${NameChain}.isConnected()
...
```typescript
${NameChain}.isConnected(): boolean;
```

### ${NameChain}.request(args)
...
${NameChain}.request(args: RequestArguments): Promise<unknown>;
```
...
### ${NameChain}.bnbSign(address: string, message: string): Promise<{publicKey: string, signature: string}>

*We prepared an example for this API, please check out:https://github.com/${githubusername}/js-eth-personal-sign-examples for more detail*

Like `eth_sign` enable dapp to verify a user has control over an ethereum address by signing an arbitrary message. We provide this method for dapp developers to request the signature of arbitrary messages for ${Name Chain} (${SYMBOL}) and ${Name} Smart Chain (W${SYMBOL}).

If `address` parameter is a ${name chain} address (start with `bnb` or `tbnb`), we will simply **calculate sha256 hash of the message** and let user sign the hash with his ${name chain} address' private key. Note: ${Name Chain} uses the same elliptic curve (`secp256k1`) as Ethereum.

If `address` parameter is a ${name} smart chain address (start with `0x`), the message would be hashed in the same way with [`eth_sign`](https://eth.wiki/json-rpc/API#eth_sign).

The returned `publicKey` would be a compressed encoded format (a hex string encoded 33 bytes starting with "0x02", "0x03") for ${Name Chain}. And uncompressed encoded format (a hex string encoded 65 bytes starting with "0x04").

The returned `signature` would be bytes encoded in hex string starting with `0x`. For ${Name Chain}, its r,s catenated 64 bytes in total. For ${Name} Smart Chain, like `eth_sign`, its r, s catenated 64 bytes and a recover byte in the end.

!!! warning

    DApp developers should verify whether the returned public key can be converted into the address user claimed in addition to an ECDSA signature verification. Because any plugin can inject the same object `${NameChain}` as ${Name Chain} Wallet.

### ${NameChain}.switchNetwork(networkId: string): Promise<{networkId: string}>

As ${Name Chain} Wallet natively supports ${Name Chain} and ${Name} Smart Chain which are heterogeneous blockchains run in parallel. APIs would be different in any situation. (We would open APIs for ${Name Chain} very soon).

Developers could judge which network is selected by user currently via `${NameChain}.chainId` or listening to the `chainChanged` event via `${NameChain}.on('chainChanged', callback)`.

To request for network switching, developers could invoke this API with `bbc-mainnet` (${Name Chain} Main Network), `${name}-mainnet` (${Name} Smart Chain Main Network), `bbc-testnet` (${Name Chain} Test Network), `${name}-testnet` (${Name} Smart Chain Test Network) to prompt user to agree on network switching.

### ${NameChain}.requestAccounts()

Request all accounts maintained by this extension.

The `id` in response would be used as `accountId` for the APIs for ${Name Chain}. 
...
### ${NameChain}.transfer({fromAddress:string, toAddress:string, asset:string, amount:number, memo?: string, sequence?: number, accountId:string, networkId:string})>

Transfer certain `amount` of `asset` (${SYMBOL} or ${TOKENSTANDARD}2) on ${Name Chain}.

`accountId` could be retrieved from the `${NameChain}.requestAccounts` API (`id` field of each account)

`networkId` could be `bbc-mainnet` or `bbc-testnet`

For example:

1. This will ask the user's approval for transferring 1 ${SYMBOL} to himself.
`${NameChain}.transfer({fromAddress:"tbnb1sndxdzsg42jg8lc0hehx8dzzpyfxrvq937mt0w", toAddress:"tbnb1sndxdzsg42jg8lc0hehx8dzzpyfxrvq937mt0w", asset:"${SYMBOL}", amount:1, accountId:"fba0b0ce46c7f79cd7cd91cdd732b6c699440acf8c539d7e7d753d38c9deea544230e51899d5d9841b8698b74a3c77b79e70d686c76cb35dca9cac0e615628ed", networkId:"bbc-testnet"})`
...
```javascript
${NameChain}.on('accountsChanged', (accounts) => {
  // Handle the new accounts, or lack thereof.
  // "accounts" will always be an array, but it can be empty.
});

${NameChain}.on('chainChanged', (chainId) => {
...
${NameChain}.on('connect', handler: (connectInfo: ConnectInfo) => void);
```
...
```typescript
${NameChain}.on('disconnect', handler: (error: ProviderRpcError) => void);
```
...
```typescript
${NameChain}.on('accountsChanged', handler: (accounts: Array<string>) => void);
```
...
```typescript
${NameChain}.on('chainChanged', handler: (chainId: string) => void);
```

```javascript
${NameChain}.on('chainChanged', (_chainId) => window.location.reload());
```
...
${NameChain}.on('message', handler: (message: ProviderMessage) => void);
```
...
- Detect which ${NameChain} network the user is connected to
- Get the user's ${NameChain} account(s)
...
let currentChainId = ${NameChain}.chainId;

${NameChain}.on('chainChanged', handleChainChanged);
...
// connected.
${NameChain}.on('accountsChanged', handleAccountsChanged);

// For now, 'eth_accounts' will continue to always return an array
function handleAccountsChanged(accounts) {
  if (accounts.length === 0) {
    // ${Name Chain} Wallet is locked or the user has not connected any accounts
    console.log('Please connect to ${Name Chain} Wallet.');
...
To be compatible with existing dApps that support MetaMask, ${Name Chain} Wallet implements them as well, but please don't rely on them. We may deprecate them soon in the future.

## Legacy Properties

### ${NameChain}.networkVersion (DEPRECATED)
...
### ${NameChain}.selectedAddress (DEPRECATED)
...
### ${NameChain}.sendAsync() (DEPRECATED)
...
${NameChain}.sendAsync(payload: JsonRpcRequest, callback: JsonRpcCallback): void;
```

### ${NameChain}.send() (DEPRECATED)
...
It is essentially an overloaded version of [`${NameChain}.sendAsync()`](#${NameChain}-sendasync-deprecated).

`${NameChain}.send()` can be called in three different ways:

```typescript
// 1.
${NameChain}.send(payload: JsonRpcRequest, callback: JsonRpcCallback): void;

// 2.
${NameChain}.send(method: string, params?: Array<unknown>): Promise<JsonRpcResponse>;

// 3.
${NameChain}.send(payload: JsonRpcRequest): unknown;
```

You can think of these signatures as follows:

1. This signature is exactly like `${NameChain}.sendAsync()`

2. This signature is like an async `${NameChain}.sendAsync()` with `method` and `params` as arguments, instead of a JSON-RPC payload and callback
...
```typescript
${NameChain}.on('close', handler: (error: Error) => void);
```
...
```typescript
${NameChain}.on('networkChanged', handler: (networkId: string) => void);
```
...
```typescript
${NameChain}.on('notification', handler: (payload: any) => void);
```
