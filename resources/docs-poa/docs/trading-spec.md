# ${Name} DEX Trading Specification

## Orders

Orders are the requests for client to buy or sell tokens into other tokens on ${Name} DEX.
It is a standard type of ${Name Chain} transaction. Orders are composed of the below parameters.

0. Symbol Pairs: the list pair the order wants to trade.
1. Order Type: ${Name} DEX only accept LIMIT orders, which is adhering to SEC definitions of LIMIT orders
...
2. the ${Name} DEX would try to match the order against any existing orders or new orders from the same block.
...
!!! Tip
        As discussed in [BEP-67](https://github.com/${githubusername}/${TOKENSTANDARD}s/blob/master/${TOKENSTANDARD}67.md), those orders in the best 500 price levels on both ask and bid side will be expired after **30 days** instead of 72 hours. Meanwhile, the expiration fee is unchanged. ${TOKENSTANDARD}67 is already implemented and has been activated after Testnet Nightingale Upgrade. ${Name Chain} Mainnet will be upgraded to support BEP-67 soon.
...
${SYMBOL} is the priority in the fee collection and has some discounts.
...
View system fees updated in real time [here](https://dex.${domainchain}/api/v1/fees).


### Multi-send Fees
`eth-cli`  offers you a multi-send command to transfer multiple tokens to multiple people. 20% discount is available for `multi-send` transactions. For now, `multi-send` transaction will send some tokens from one address to multiple output addresses. If the count of output address is bigger than the threshold, currently it's 2, then the total transaction fee is  0.0003 ${SYMBOL} per token per address.
...
[
   {
      ...
      "amount":"100000000:${SYMBOL},100000000:ABC"
   },
   {
   ...
   },
   {
      ...
      "amount":"100000000:${SYMBOL},100000000:SAT"
   }
]
```
...
Transaction Type | Pay in non-${SYMBOL} Asset | Pay in ${SYMBOL}
-- | -- | --
Trade | 0.1% | 0.05%

Trading fee can be queried at [here](https://dex.${domainchain}/api/v1/fees?format=amino). It's under the "params/DexFeeParam/".  "FeeRate" and "FeeRateNative" are both under unit of 10^-6.
...
View system fees updated in real time [here](https://testnet-dex.${domainchain}/api/v1/fees).
...
- Non-Trade related transactions will be charged a fee when the transactions happen, and can only be paid in ${SYMBOL}. The transaction will be rejected if the address does not have enough ${SYMBOL}.

- Trade related transaction would be charged with fee when an order is filled, or canceled/expired/IOC-expired with no fills. If there is enough ${SYMBOL} to pay, ${SYMBOL} fee structure would be used, otherwise, non-${SYMBOL} fee structure would be used to charged.
