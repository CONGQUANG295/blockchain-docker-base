# Mirror ${TOKENSTANDARD}2 and ${TOKENSTANDARD}20 Token

## Prerequisite

This ${TOKENSTANDARD}20 token is not bond to any ${TOKENSTANDARD}2 token

## Motivation

Anyone can call the `mirror` [method](https://github.com/${githubusername}/${name}-genesis-contract/blob/af4f3993303213052222f55c721e661862d19638/contracts/TokenManager.sol#L331) to issue a ${TOKENSTANDARD}2 token automatically and bind them.

## What happens under the hood

- Verify there is no pending mirror request
- Check the total supply and token symbol is valid
- Send a cross-chain package to issue a ${TOKENSTANDARD}2 token on ${Name Chain}
- The newly created ${TOKENSTANDARD}2 token is locked until token holder send cross-chain transfer

After binding, all liquid circulation is on ${Name}.

## Fee Table

| Fee Name    | Pay in ${SYMBOL} |
| ----------- | ---------------------------- |
| mirrorFee   | it's 10 ${SYMBOL} on mainnet now   |
...
- Select `mirror` function and fill-in with your ${TOKENSTANDARD}20 address
...
## Query ${TOKENSTANDARD}2 Token Symbol

You can query ${TOKENSTANDARD}2 Token symbol from `tokenhub` contract.

Use the latest [ABI](https://raw.githubusercontent.com/${githubusername}/${name}-genesis-contract/master/abi/tokenhub.abi)

Select `getBound${Tokenstandard}2Symbol` function
