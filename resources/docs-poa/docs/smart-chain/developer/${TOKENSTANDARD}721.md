...
${TOKENSTANDARD}721 is a standard for representing ownership of [non-fungible tokens](https://docs.openzeppelin.com/contracts/3.x/tokens#different-kinds-of-tokens), that is, where each token is unique such as in real estate or collectibles.

We will use [Presets](https://docs.openzeppelin.com/contracts/3.x/api/presets) contracts in [OpenZeppelin Contracts](https://docs.openzeppelin.com/contracts/3.x/) to create an ${TOKENSTANDARD}721 and deploy using Truffle.
...
Then we install [OpenZeppelin Contracts](https://docs.openzeppelin.com/contracts) which has an implementation of ${TOKENSTANDARD}721.
...
We are going to use Preset [`ERC721PresetMinterPauserAutoId`](https://docs.openzeppelin.com/contracts/3.x/api/presets#ERC721PresetMinterPauserAutoId) which is an ${TOKENSTANDARD}721 that is preset so it can be minted (with auto token ID and metadata URI), paused and burned.
...
- Head over to [Faucet](https://faucet.${domainchain}) and request test ${SYMBOL}
...
Next we will deploy to ${Name Chain}  testnet .
...
```
     testnet: {
      provider: () => new HDWalletProvider(mnemonic, `https://testnet-rpc.${domainchain}`),
      network_id: ${TestnetId},
      confirmations: 10,
      timeoutBlocks: 200,
      skipDryRun: true
    },
    ${name}: {
      provider: () => new HDWalletProvider(mnemonic, `https://mainnet-rpc.${domainchain}`),
