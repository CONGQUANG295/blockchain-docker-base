...
${SYMBOL} and coins on ${Name Chain} are supported by the Ledger Nano S hardware wallet. The following describes how to use Ledger Nano S to connect to ${Name}
...
!!! Tip
		The existing Ethereum App is compatible with ${Name Chain}
...
2) Within the "Manager" pane, type in "Ethereum" in the search field. Locate "Ethereum", then click on "Install". (Note: Do not use the "${NameChain}" App)
...
## Enabling contract data for ${TOKENSTANDARD}20 

!!! Tip
		 Please ensure your enabled contract data before transfering ${TOKENSTANDARD}20 tokens 
...
${Name Chain} is compatible with MetaMask. Users can use MetaMask as a bridge between Ledger Nano S and ${Name Chain} to make transactions to the ${Name Chain} testnet.

1) Connect [MetaMask](./metamask.md) to ${Name} Testnet by following this guide to add a custom network

 * Testnet
    * [RPC URLs](../developer/rpc.md)
    * ChainID: ${HexTestnetId} (${TestnetId} in decimal)
    * Symbol: ${SYMBOL}
    * [Block Explorer](https://testnet.bscscan.com)

 * Mainnet
    * [RPC URLs](../developer/rpc.md)
    * ChainID: ${HexMainnetId} (${MainnetId} in decimal)
    * Symbol: ${SYMBOL}
...
1. Fail to tranfer ${TOKENSTANDARD}20 tokens
...
```
You need to follow [these steps]((#enabling-contract-data-for-${TOKENSTANDARD}20)) to enable contract data. 
