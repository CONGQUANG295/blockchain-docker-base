# Run a light client to join ${Name Chain}

Light client is a program that connects to a full node to help users access and interact with ${Name Chain}
...
Download:
```bash
git clonehttps://github.com/${githubusername}/node-binary.git
```
...
```
./lightd --help
This node will run a secure proxy to a ${name} rpc server.

All calls that can be tracked back to a block header by a proof
will be verified before passing them back to the caller. Other that
that it will present the same interface as a full ${name} node,
...
Flags:
      ...
      --chain-id string            Specify the ${name} chain ID (default "bnbchain")
  -h, --help                       help for lite
      --home-dir string            Specify the home directory (default ".${name}-lite")
      ...
      --node string                Connect to a ${name} node at this address (default "tcp://localhost:27147")
```
...
You can find chain id at [genesis file in test network](https://github.com/${githubusername}/node-binary/blob/master/fullnode/testnet/0.5.8/config/genesis.json)
or [genesis file in prod network](https://github.com/${githubusername}/node-binary/blob/master/fullnode/prod/0.5.8/config/genesis.json).<br/>
The `full node addr` field can be an address of any full node that you have deployed.<br/>
You can refer to [Run a ${Name Chain} full node](fullnode.md) to get more details.<br/>
...
d = requests.get('https://dex.${domainchain}/api/v1/peers').text # replace dex.${domainchain} with testnet-dex.${domainchain} for testnet
l = json.loads(d)
...
### Example for Mainnet:
```bash
./lightd --chain-id "${Name-Chain}-Tigris" --node tcp://dataseed1.${domainchain}:80 > node.log  &
```

### Example for Testnet:
```bash
./lightd --chain-id "${Name-Chain}-Ganges" --node tcp://data-seed-pre-0-s1.${domainchain}:80 > node.log  &
```
