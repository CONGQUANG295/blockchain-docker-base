...
## Bridge Module
Bridge module will process cross chain transactions. It contains two parts: transactions from bc to ${name} and transactions from ${name} to bc.
For transactions from ${name} to bc, it will depend on the oracle module. When the validators reach a consensus on a certain claim, the bridge module will process the transaction according to the claim, like transfer from ${name} to bc.
For transactions from bc to ${name}, it will process the bc part of the transaction and write the related cross chain package for ${name}.
