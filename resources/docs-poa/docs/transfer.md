...
`Transfer` is the most basic transaction ${Name Chain} supports, it moves assets among different addresses.

Please note the fees must be paid first in ${SYMBOL} before the transaction can be executed.
...
```bash
 ./eth-cli account bnb1XXXXXXXXXXXXXXXXXXX --chain-id ${Name-Chain}-Tigris --node  https://dataseed5.${domainchain}:443 --indent
{
...
```bash
./eth-cli account tbnb1sylyjw032eajr9cyllp26n04300qzzre38qyv5 --chain-id ${Name-Chain}-Ganges --node=data-seed-pre-2-s1.${domainchain}:80 --indent
...
```bash
./eth-cli send --from from-key-name --to to-address --amount 200000000:${SYMBOL} --chain-id ${Name-Chain}-Tigris --node  https://dataseed5.${domainchain}:443 --json --memo "Test transfer"
...
Example on **testnet**:
```bash
./eth-cli send --from from-key-name --to to-address --amount 200000000:${SYMBOL} --chain-id=${Name-Chain}-Ganges --node=data-seed-pre-2-s1.${domainchain}:80 --json --memo "Test transfer"
...
Flags:
      ...
      --chain-id string      Chain ID of ${Name Chain} node
...
Flags:
  ...
  -h, --help                    help for multi-send
      ...
      --sequence int            Sequence number to sign the tx
      --source int              Source of tx
      --transfers string        Transfers details, format: [{"to": "addr", "amount": "1:${SYMBOL},2:BTC"}, ...]
...
```bash
 ./eth-cli token multi-send --home ./testnodecli --from test --chain-id ${Name-Chain}-Tigris --node  https://dataseed5.${domainchain}:443  --transfers "[{\"to\":\"bnb1g5p04snezgpky203fq6da9qyjsy2k9kzr5yuhl\",\"amount\":\"100000000000000:${SYMBOL}\"},{\"to\":\"bnb1l86xty0m55ryct9pnypz6chvtsmpyewmhrqwxw\",\"amount\":\"100000000000000:${SYMBOL}\"}]" --json
Password to sign with 'test':
{
   "Height":"1412",
...
./eth-cli token multi-send --home ./testnodecli --from test --chain-id=${Name-Chain}-Ganges --node=data-seed-pre-2-s1.${domainchain}:80  --transfers "[{\"to\":\"tbnb1sylyjw032eajr9cyllp26n04300qzzre38qyv5\",\"amount\":\"100000000000000:${SYMBOL}\"},{\"to\":\"tbnb1e244vmvym7g6cn9lk4hmhf9p2f9jaf0x9hxmwc\",\"amount\":\"100000000000000:${SYMBOL}\"}]" --json
...
```bash
 ./eth-cli token multi-send --home ./testnodecli --from test --chain-id ${Name-Chain}-Tigris --node https://dataseed5.${domainchain}:443  --transfers-file ./transaction.json --json
Password to sign with 'test':
{
   "Height":"1412",
   "TxHash":"A238C3C33625B5398FE648BD3FE9822CB7A07A2DB7778376546916F81C634138",
...
Example on **testnet**:
```bash
 ./eth-cli token multi-send --home ./testnodecli --from test--chain-id=${Name-Chain}-Ganges --node=data-seed-pre-2-s1.${domainchain}:80  --transfers-file ./transaction.json --json
