...
Introducing [${Name} Bridge V2](https://www.${domainchain}/en/bridge), a brand new bridge service providing access to inter-blockchain liquidity for all popular blockchain networks, brings valuable assets such as BUSD, USDT, and ETH to ${Name Chain} ecosystems. 

In the [first version](https://docs.${domainchain}/smart-chain/guides/panama.html), the procedure of swapping tokens has two categories: *Peg-In* and *Peg-Out*. Peg-In allows users to exchange the native tokens to the equivalent of pegged tokens on ${Name Chain} or ${Name} Smart Chain. Meanwhile, the Peg-Out procedure enables users to swap the equivalent of pegged tokens on ${Name Chain} or ${Name} Smart Chain to the native tokens. 

Many more networks are coming soon ~


## User Guide
### Swap Tokens to ${Name Chain} from a different Network
- Install your extension wallet; it can be ${Name} Extension Wallet or MetaMask
- Go to ${Name} Bridge page, and you will use V2 by default.
...
${Name} Bridge supports 35 popular cryptocurrencies. You can move your USDT, BUSD, or ETH across different blockchains quickly. You can see the whole list from this API: https://api.${domainchain}/bridge/api/v2/tokens 
...
To swap Tokens to ${Name Chain} from a different network, the destination network must be ${Name Chain} or ${Name} Smart Chain. You can click on the arrow button to switch between these two networks. 
...
### Swap Tokens from ${Name Chain} to a different Network
...
To swap Tokens from ${Name Chain} to a different Network, the source network must be ${Name Chain}, ${Name} Smart Chain or Ethereum (with metamask)
...
${Name} Bridge is the safest, fastest, and most secure way to bring cross-chain assets to ${Name Chain} and ${Name} Smart Chain. If you have any issues, please contact customer support of [${Domainchain}](https://www.${domainchain}/en/support).
...
2. What is pegged Bitcoin on ${Name Chain} and ${Name} Smart Chain?

${Domainchain} announced  [Project Token Canal](https://www.${domainchain}/en/blog/${name}-presents-project-token-canal-2/), which is a new initiative, similar to many existing ‘wrapped coins’ in the crypto community. As the largest crypto token vault and exchange, ${Domainchain} will issue and bind more token assets on ${Name Chain} and ${Name} Smart Chain, and guarantee the conversion from and to the original tokens with credibility and infrastructure. [Announcement](https://www.${domainchain}/en/support/articles/daca7c991d5f4c45a4d1083f70912515) [Project Token Canal](https://www.${domainchain}/en/blog/${name}-presents-project-token-canal-2/) is a great option for users of ${Domainchain}, while ${Name} Bridge is the powerful alternative for non-${Domainchain} users and more. These two channels will work together to provide more liquidity to ${Name Chain} ecosystem. [View Proof of Assets](https://www.${domainchain}/en/assets-proof) 
