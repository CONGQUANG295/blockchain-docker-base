...
Relayers for BC-to-${Name} communication referred to as **${Name} Relayers** are a standalone process that can be run by anyone, and anywhere, except that Relayers must register themselves onto ${Name} and deposit a certain amount of ${SYMBOL}. Only relaying requests from the registered Relayers will be accepted by ${Name}.

GitHub Implementation link: <https://github.com/${githubusername}/${name}-relayer>

Config Files: <https://github.com/${githubusername}/${name}-relayer-config>

## Monitor and Parse Cross Chain Event
As a ${Name} relayer, it must have proper configurations on the following three items:

| Name | Type | Description |
| ---- | ---- | ----------- |
|srcCrossChainID | uint16 | CrossChainID of BC, the value is 1 for testnet |
|destCrossChainID| uint16 | CrossChainID of ${Name}, the value is ${TestnetId} for testnet |

A ${Name} relayer is required to parse all block results and pick out all events with event type “IBCPackage” from endBlock event table. This is an cross chain package event example:
...
${Name} relayer should iterate all the attributes and parse the attribute value:
...
| ---- | ------ | ------ |
|prefix|1 bytes|0x00|
|source chain CrossChainID|2 bytes|srcCrossChainID in ${name} relayer configuration|
|destination chain CrossChainID|2 bytes|destCrossChainID in ${name} relayer configuration|
...
[${Name} Relayer Incentive Mechanism](incentives.md)
