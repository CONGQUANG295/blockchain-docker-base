...
When users swap their tokens to the ${TOKENSTANDARD}20 equivalents on ${Name Chain}, they also need to get some ${SYMBOL} to pay for gas.

## Requirements

* The destination network has to be **${Name Chain}**
* The token is a popular BToken, such as BTC, ETH, USDT, BUSD
* The total of your swap order has to be greater than **2${SYMBOL}**

### How many ${SYMBOL} you can get?

There are 3 options: 0.5 ${SYMBOL}, 1${SYMBOL} and 2 ${SYMBOL}
...
### 2. Check the box "I want to swap some ${SYMBOL} gas in this order"

### 3. Choose ${SYMBOL} amount
...
The network fee should be the sum of the fee for swapping assets and the fee for purchasing ${SYMBOL} on your behalf.
...
Received token  = Input amount  - network fee - cost to buy ${SYMBOL}
