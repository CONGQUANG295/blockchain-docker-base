...
The ${Name} relayers play an important role in relaying interchain packages from BC to ${Name}.
All ${Name} relayers build their stable infrastructure, watch any event happened on the ${Name Chain}, and act timely to get paid accordingly. The following discussion is about how to distribute the rewards to let the relayers are willing to make a long-term contribution.
...
1. Users paid reward: Users who send `bind` or `cross chain transfer` transactions need to pay extra fee as ${name}-relayer rewards.
...
| --------------------------------------------------- | ------------- |
|User packages(bind, unbind, transfer) from BC to ${Name} | Users pay for the reward |
|System packages(staking, slash, governance) from BC to ${Name} | System reward |
...
3. The relayFee of a single package and the ratio of reward for header relayer can be modified by governance on ${Name Chain}.
...
The system reward pool can hold at most 100 ${SYMBOL} for example, to prevent the pool get unnecessary income.
...
For example, a relayer may deliver packages using a different address in round robin, we can’t recognize this. We try to introduce registration and ${SYMBOL} deposit for relayer to raise the cost of cheat.  How it works:

* A ${Name} account needs call `register` of [RelayerHub](https://explorer.${domainchain}/address/0x0000000000000000000000000000000000001006) contract to deposit 100 ${SYMBOL}(more or less than 100 ${SYMBOL} will be rejected) to become a ${Name} relayer.
* Only a valid relayer can sync ${Name Chain} Headers and deliver cross-chain packages.
* Relayer can withdraw its deposit, but we will charge 0.1 ${SYMBOL} as the transaction fee so that it will receive 99.9 ${SYMBOL} back.
