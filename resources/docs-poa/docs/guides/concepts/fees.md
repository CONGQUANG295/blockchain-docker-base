...
**${SYMBOL}** is the native token on ${Name Chain}, thus users are charged ${SYMBOL} for sending transactions.
...
The difference between ${Name Chain} and Ethereum is that there is no notion of `gas`. As a result,
...
`eth-cli`  offers you a multi-send command to transfer multiple tokens to multiple people. 20% discount is available for `multi-send` transactions. For now, `multi-send` transaction will send some tokens from one address to multiple output addresses. If the count of output address is bigger than the threshold, currently it's 2, then the total transaction fee is  0.001 ${SYMBOL} per token per address.
...
you can use [API](https://dex.${domainchain}/api/v1/fees) to get the latest fee params.
```json
[
{
...
The rewards for ${Name Chain} validators are displayed in explorer. For example: in block [59947302](https://explorer.${domainchain}/block/59947302), validator [bnb1tpagqqqx36gq09kzw4f5a3a9sk3tq54dpl5ldn](https://explorer.${domainchain}/address/bnb1tpagqqqx36gq09kzw4f5a3a9sk3tq54dpl5ldn) get `0.00005 ${SYMBOL}` as rewards.
...
```json
{"Height":59947302,"Fee":"${SYMBOL}:5000","Validators":["bnb1tpagqqqx36gq09kzw4f5a3a9sk3tq54dpl5ldn"]}
{"Height":59947303,"Fee":"${SYMBOL}:5000","Validators":["bnb1y888axmhzz6yjj464syfy68mkhzy9phlv8fzac"]}
{"Height":59947304,"Fee":"${SYMBOL}:5000","Validators":["bnb19klje94mnu53wj7pmrk0zmtpwgr0uz8th0fcvw"]}
{"Height":59947305,"Fee":"${SYMBOL}:21364","Validators":["bnb19hunw9ps8n9tkrp2j64jvheezgqmfc2eyrxd7a"]}
...
...
```
Trading fees can be charged in different ${TOKENSTANDARD}2 tokens,
