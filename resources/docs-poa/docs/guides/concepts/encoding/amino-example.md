...
${Name Chain} (Amino) encoding logic may encode a data structure into two output formats: Binary and JSON.
...
Let's take a look at an [example function](https://github.com/${githubusername}/go-sdk/blob/fe7524098533a6ce6077cf5f28f677329963d17b/client/rpc/dex_client.go#L373) of ${Name Chain} go-sdk.
...
Let's take a look at an [example function]() of ${Name Chain} go-sdk.   The signed transaction needs to be amino-encoded, thus ready to be posted to BncChain API.
...
## ${Name Chain} Transaction Encoding

Below are the data types that can be sent to ${Name Chain} as requests, and their encoding layout. To simplify the presentation, we will use a variant of [Google protocol buffer proto3](https://developers.google.com/protocol-buffers/docs/proto3) language to illustrate how the data fields are organized. Except the all-capitalized fields, other fields will use the default `proto3` encoding logic.
...
#### NewOrder
NewOrder transaction will create a new order to buy or sell tokens on ${Name} DEX.
...
##### Order ID
Order ID is unique across the world. It is generated by sender and acknowledged by ${Name} DEX. The current implementation is composed from 3 parts:
...
#### Cancel
Cancel transactions (cancel the outstanding/unfilled) orders from the ${Name} DEX. After cancel success, the locked quantity on the orders will return back to the originating address balance and become free to use, i.e. transfer or send new orders.
...
#### Issue
Issue (create) new asset on ${Name Chain}.
```go
