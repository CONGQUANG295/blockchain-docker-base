...
## Upcoming Changes in ${Name Chain} Mainnet API - 2021-06-04
...
## Upcoming Changes in ${Name Chain} Mainnet API v0.7.2

### Node RPC

Extra paths are available for querying information about `${TOKENSTANDARD}8` tokens and trading pairs.
...
**URL for testnet:** [https://dex.${domainchain}/api/v1/depth?symbol=xxx-000_${SYMBOL}](https://dex.${domainchain}/api/v1/depth?symbol=xxx-000_${SYMBOL})

**Parameters**

| Name | Located in | Description | Required | Schema |
| ---- | ---------- | ----------- | -------- | ---- |
| symbol | query | Market pair symbol, e.g. NNB-0AD_${SYMBOL} | Yes | string |
...
| Name | Type | Description | Example |
| ---- | ---- | ----------- | ------- |
| name | string |  | ${Name Chain} Mini Token |
...
| Name | Type | Description | Example |
| ---- | ---- | ----------- | ------- |
| base_asset_symbol | string (currency) | symbol of base asset | ${SYMBOL} |
...
## Upcoming Changes in ${Name Chain} Testnet API v0.7.0
...
**URL for testnet:** [https://testnet-dex.${domainchain}/api/v1/depth?symbol=xxx-000_${SYMBOL}](https://testnet-dex.${domainchain}/api/v1/depth?symbol=xxx-000_${SYMBOL})
...
## Upcoming Changes in ${Name Chain} API v0.6.4
...
**Example**
```
https://testnet-dex-asiapacific.${domainchain}/api/v1/account/tbnb10qpmrlsr4mq65xwgjd39xypkkpw3wm9c5e58xm
```
Return:
```
{
...
Add `buyerSource` and `sellerSource` in the response Trade data structure. The Source Id reference can be found inhttps://github.com/${githubusername}/${TOKENSTANDARD}s/blob/master/${TOKENSTANDARD}10.md.
...
| buySingleFee | string | trading fee for the buyer address on this single trade | ${SYMBOL}:0.00000172; |
...
| sellSingleFee | string | trading fee for the seller address on this single trade | ${SYMBOL}:0.00000216; |
...
## Upcoming Changes in ${Name Chain} API v0.6.3
...
As described in https://docs.${domainchain}/api-reference/dex-api/ws-streams.html#6-book-depth-streams, you can now customize the returned level from orderbook. The default level is 20 and you can extend the level to 100, 500, or 1000 with compression enabled.

* Example on mainnet:
```
var  marketDepth = new WebSocket("wss://dex-atlantic.${domainchain}/api/ws/NEXO-A84_${SYMBOL}@marketDepth100");
```
* Example on testnet:
```
var  marketDepth = new WebSocket("wss://testnet-dex.${domainchain}/api/ws/ALT-3B6_${SYMBOL}@marketDepth100");
```
...
## Upcoming Changes in ${Name Chain} API v0.6.2
...
## Upcoming Changes in ${Name Chain} API v0.6.1

### HTTP API

As you know, There are some accelerate nodes which provides some advanced API services for the public. Here is a list of all the HTTP API information it provides on mainnet: https://docs.${domainchain}/api-reference/dex-api/paths.html
...
## Upcoming Changes in ${Name Chain} API v0.6.0
...
All those [extra info](https://docs.${domainchain}/get-extra-data-from-fullnode.html#publish-different-messages-to-local-files ) can also be found in exported data from your fullnode.
