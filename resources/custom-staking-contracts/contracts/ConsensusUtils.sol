...
import "./vendor/abstracts/ValidatorSet.sol";
...
contract ConsensusUtils is EternalStorage, ValidatorSet {
  ...
  uint256 public constant MAX_STAKE = 300e24; // 300,000,000 GTBS
  ...
  */
  ...
  event MinDelegationUpdated(uint256 value);
  event MaxDelegationPerWalletUpdated(uint256 value);
  ...
  */
  ...
  bytes32 internal constant MIN_DELEGATION = keccak256(abi.encodePacked("minDelegation"));
  bytes32 internal constant MAX_DELEGATION_PER_WALLET = keccak256(abi.encodePacked("maxDelegationPerWallet"));
  ...
  function initializeCustomParams(uint256 _minDelegation, uint256 _maxDelegationPerWallet) external onlyOwner {
    require(_minDelegation > 0);
    require(_maxDelegationPerWallet >= _minDelegation);
    uintStorage[MIN_DELEGATION] = _minDelegation;
    uintStorage[MAX_DELEGATION_PER_WALLET] = _maxDelegationPerWallet;
  }

  function minDelegation() public view returns (uint256) {
    return uintStorage[MIN_DELEGATION];
  }

  function maxDelegationPerWallet() public view returns (uint256) {
    return uintStorage[MAX_DELEGATION_PER_WALLET];
  }

  function setMinDelegation(uint256 _v) external onlyOwner {
    require(_v >= 1e21 && _v <= 1e24);
    uintStorage[MIN_DELEGATION] = _v;
    emit MinDelegationUpdated(_v);
  }

  function setMaxDelegationPerWallet(uint256 _v) external onlyOwner {
    require(_v >= minDelegation());
    uintStorage[MAX_DELEGATION_PER_WALLET] = _v;
    emit MaxDelegationPerWalletUpdated(_v);
  }
