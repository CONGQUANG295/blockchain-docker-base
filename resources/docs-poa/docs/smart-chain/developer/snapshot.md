...
* [Latest Snapshot](https://github.com/${githubusername}/${name}-snapshots)

The following commands are step by step instructions for ${Name} node operators that can be used for two major use-cases:
...
```
wget --no-check-certificate --no-proxy 'https://raw.githubusercontent.com/${githubusername}/${name}-snapshots/master/chaindata_latest.zip'
```
...
```
# Extract the data
nohup unzip  /NAME_OF_YOUR_HOME/node/geth/ -f chaindata_latest.zip &
