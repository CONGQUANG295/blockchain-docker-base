...
## Delegate with ${Name} Extension Wallet

Detailed user guide is [here](../wallet/staking.md)

### Create an account if necessary

Please refer to this guide on [how to create a wallet](../wallet/${name}.md) with ${Name} Extension Wallet

### Go to the staking page:

- Mainnet: <https://www.${domainchain}/en/staking>
- Testnet: <https://testnet-staking.${domainchain}/en/staking >
...
### Swtich to ${Name Chain} Network

If you are using ${Name Chain}, please follow these steps to switch network:
...
## Delegate with ${Name Chain} Command line
...
Fee for `Delegate Smart Chain Validator`is **0.001${SYMBOL}**

The minimum delegated amount is **1${SYMBOL}**.
...
```bash
## mainnet
eth-cli staking side-top-validators --top 10 --side-chain-id=${name} --chain-id=${Name-Chain}-Tigris

## testnet
eth-cli staking side-top-validators --top 10 --side-chain-id=chapel --chain-id=${Name-Chain}-Ganges
```

## Delegate ${SYMBOL}

You can use `eth-cli` or `eth-cli` to [delegate](../../guides/concepts/bc-staking.md#delegate-bnb) some of ${SYMBOL} to a validator
...
You can use `eth-cli`or `eth-cli` to [undelegate](../../guides/concepts/bc-staking.md#undelegate-bnb) some bonded ${SYMBOL} from a validator

## Redelegate ${SYMBOL}

You can use `eth-cli`or `eth-cli` to [redelegate](../../guides/concepts/bc-staking.md#redelegate-bnb) some bonded ${SYMBOL} from a validator
...
Please refer to this [guide](../wallet/${name}.md#${name}.html#transfer-testnet-${symbol}-from-${name}-to-bc)
