...
${Name Chain} has its own built-in governance module that lets ${SYMBOL} holders submit proposals for adding trade pairs. In order for the proposal to be open for voting, it needs to come with a `deposit` that is greater than a parameter called `Deposit`. The `deposit` need not be provided in its entirety by the submitter. If the initial proposer's `deposit` is not sufficient, the proposal enters the `deposit_period` status. Then, any ${SYMBOL} holder can increase the deposit by sending a `depositTx`.
A purely-code-controlled escrow account will be used to hold deposits during voting period. It is a kind of account which is derived from a hard-coded string in ${name} chain protocol. This kind of account doesn't have its own private key and it's only controlled by code of the protocol. The code for calculating escrow account is the same that is used in [cosmos-sdk](https://github.com/cosmos/cosmos-sdk/blob/82a2c5d6d86ffd761f0162b93f0aaa57b7f66fe7/x/supply/internal/types/account.go#L40):
```
DepositedCoinsAccAddr = sdk.AccAddress(crypto.AddressHash([]byte("${NameChain}DepositedCoins")))
```
...
### Global Parameters
* `min-deposit`: The threshold for submitting a proposal on mainnet is **1000${SYMBOL}**, and the threshold for submitting a proposal on testnet is **2000${SYMBOL}**
* `deposit_period`: This is a global parameter and the value for mainnet is two days and testnet is two week. It means the time to deposit enough ${SYMBOL} tokens is two days in mainnet and two weeks in testnet.
...
Please use this [tool](<https://github.com/${githubusername}/node-binary/tree/master/tools>) for generating

You can also use [Web Wallet](https://community.${domainchain}/topic/2487/how-to-manage-your-${tokenstandard}2-token-on-${name-chain}-in-web-wallet) for token management operations.

#### Submit a List Proposal
To add a new trading pairs, you can run the following command:<br/>
Please note:<br/>

+ `--init-price` is boosted by **1e8** for decimal part, such as 100000000, is 1 ${SYMBOL}
...
+ `--quote-asset-symbol`: the quote asset symbol. When you first list a ${TOKENSTANDARD}2 token on ${Name} DEX, it has to be put in ${SYMBOL} market first. Thus, you should set `quote-asset-symbol` to **${SYMBOL}** in your first proposal, then your ${TOKENSTANDARD}2 asset can be list against [BUSD-BD1](https://explorer.${domainchain}/asset/BUSD-BD1) and other stablecoins.
...
```shell
./eth-cli gov submit-list-proposal --from test --deposit 100000000000:${SYMBOL}
--base-asset-symbol AAA-254 --quote-asset-symbol ${SYMBOL} --init-price 100000000 --title "list AAA-254/${SYMBOL}"
--description "list AAA-254/${SYMBOL}" --expire-time 1570665600 --chain-id ${Name-Chain}-Tigris --node https://dataseed5.${domainchain}:443 --voting-period 604800 --json
```

Example on **testnet**:

```shell
./eth-cli gov submit-list-proposal --from test --deposit 200000000000:${SYMBOL}
--base-asset-symbol AAA-254 --quote-asset-symbol ${SYMBOL} --init-price 100000000 --title "list AAA-254/${SYMBOL}"
--description "list AAA-254/${SYMBOL}" --expire-time 1570665600 --chain-id=${Name-Chain}-Ganges --node=data-seed-pre-2-s1.${domainchain}:80 --json --voting-period 604800
```
!!! Tip
        As discussed in [BEP-70](https://github.com/${githubusername}/${TOKENSTANDARD}s/blob/master/${TOKENSTANDARD}70.md), BUSD is one of the most influential stable coins worldwide and the most dominant stable coin on ${Name Chain}. ${TOKENSTANDARD}2 token issuers can list their assets against [BUSD-BD1](https://explorer.${domainchain}/asset/BUSD-BD1) without the requirement of creating ${SYMBOL} pair first. Listing and trading BUSD pairs on ${Name Chain} will facilitate token owners and exchange traders, making the markets more liquid and healthier.  ${TOKENSTANDARD}70 is already implemented and has been activated after Testnet Nightingale Upgrade. ${Name Chain} Mainnet will be upgraded to support BEP-70 soon.
...
```shell
eth-cli gov submit-delist-proposal --title "delist EDD-0AC" --voting-period 7200 --deposit "200000000000:${SYMBOL}" --justification " justification " --base-asset-symbol EDD-0AC --quote-asset-symbol ${SYMBOL} --from <your-key-name> --chain-id ${Name-Chain}-Tigris --node https://dataseed5.${domainchain}:443 --trust-node
```
...
```shell
eth-cli gov submit-delist-proposal --title "delist EDD-0AC" --voting-period 7200 --deposit "200000000000:${SYMBOL}" --justification " justification " --base-asset-symbol EDD-0AC --quote-asset-symbol ${SYMBOL} --from <your-key-name> --chain-id ${Name-Chain}-Ganges --trust-node --node https://seed-pre-s3.${domainchain}:443
```

#### Add deposit for a Proposal (Optional)
If the initial deposit for your proposal in `submit-list-proposal` is not enough, you can increase the deposit with `deposit` operation. In current ${Name Chain} Mainnet, the max deposit period is **two days**. After submitting a proposal, you have two days to increase your deposit, otherwise your proposal will not go into the voting period and gets rejected directly.
...
```shell
eth-cli gov deposit --from name --proposal-id <proposl-ID> --deposit <amount>:${SYMBOL} --chain-id ${Name-Chain}-Tigris --node https://dataseed5.${domainchain}:443
```

Example on **testnet**:

```shell
eth-cli gov deposit --from name --proposal-id <proposl-ID> --deposit <amount>:${SYMBOL} --chain-id=${Name-Chain}-Ganges --node=data-seed-pre-2-s1.${domainchain}:80
```
...
```shell
./eth-cli gov query-proposal --proposal-id <proposal-ID> --chain-id ${Name-Chain}-Tigris --node https://dataseed5.${domainchain}:443
```

Example on **testnet**:

```shell
./eth-cli gov query-proposal --proposal-id <proposal-ID> --chain-id=${Name-Chain}-Ganges --node=data-seed-pre-2-s1.${domainchain}:80
```
...
```json
{
  ...
  "value": {
    "proposal_id": "100",
    "title": "list CZZ-696/${SYMBOL}",
    "description": "{\"base_asset_symbol\":\"CZZ-696\",\"quote_asset_symbol\":\"${SYMBOL}\",\"init_price\":100000000,\"description\":\"list CZZ-696/${SYMBOL}\",\"expire_time\":\"2019-03-21T09:00:00+09:00\"}",
...
```shell
./eth-cli gov query-votes --proposal-id 272 --chain-id ${Name-Chain}-Tigris --node https://dataseed5.${domainchain}:443
```

Example on **testnet**:

```shell
./eth-cli gov query-votes --proposal-id 272 --chain-id=${Name-Chain}-Ganges --node=data-seed-pre-2-s1.${domainchain}:80
```
...
```bash
$ ./eth-cli gov query-proposal --chain-id ${Name-Chain}-Ganges --node=tcp://data-seed-pre-1-s3.${domainchain}:80 --proposal-id 370
{
  "type": "gov/TextProposal",
  "value": {
    "proposal_id": "370",
    "title": "list JCC-CB1/${SYMBOL}",
    "description": "{\"base_asset_symbol\":\"JCC-CB1\",\"quote_asset_symbol\":\"${SYMBOL}\",\"init_price\":100000000,\"description\":\"list JCC-CB1/${SYMBOL}\",\"expire_time\":\"2019-04-24T14:46:35+08:00\"}",
    "proposal_type": "ListTradingPair",
...
```bash
$ ./eth-cli gov query-proposal --chain-id ${Name-Chain}-Tigris --node https://dataseed5.${domainchain}:443 --proposal-id 25
{
  "codespace": 5,
  "code": 1,
  "abci_code": 327681,
