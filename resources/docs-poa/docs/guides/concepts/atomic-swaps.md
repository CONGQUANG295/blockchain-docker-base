...
Atomic swaps can be used for trading between ${TOKENSTANDARD}2 tokens, or for trading crosss different blockchains.

## Hash Timer Locked Contract

[HTLC](https://en.bitcoin.it/wiki/Hash_Time_Locked_Contracts) has been used for Atomic Swap and cross payment channel for a few years on Bitcoin and its variant blockchains, and also Ethereum. This BEP defines native transactions to support HTLC on ${Name Chain}, and also proposes the standard infrastructure and procedure to use HTLC for inter-chain atomic swap to easily create and use pegged token, which is called `Atomic Peg`.

## HTLC on ${Name Chain}


As explained in [${TOKENSTANDARD}3](https://github.com/${githubusername}/${TOKENSTANDARD}s/blob/master/${TOKENSTANDARD}3.md), Hash Timer Locked Contract(HTLC) has been used for Atomic Swap and cross payment channels between different blockchains. ${TOKENSTANDARD}3 defines native transactions to support HTLC on ${Name Chain} and also proposes the standard infrastructure and procedure to use HTLC for inter-chain atomic swap to easily create and use pegged token.
During the swap process, the related fund will be locked to a purely-code-controlled escrow account.
A purely-code-controlled escrow account is a kind of account which is derived from a hard-coded string in ${name} chain protocol. This kind of account has no its own private key and it's only controled by code in protocol. The code for calculating escrow account is the same as how it's done in [cosmos-sdk](https://github.com/cosmos/cosmos-sdk/blob/82a2c5d6d86ffd761f0162b93f0aaa57b7f66fe7/x/supply/internal/types/account.go#L40):
```
AtomicSwapCoinsAccAddr = sdk.AccAddress(crypto.AddressHash([]byte("${NameChain}AtomicSwapCoins")))
```
...
Hash Timer Locked Transfer (HTLT) is a new transaction type on ${Name Chain}, to serve as HTLC in the first step of Atomic Swap,
...
| OutAmount | Coins | similar to the Coins in the original Transfer defined in ${TOKENSTANDARD}2, assets to swap out | No |
| ExpectedIncome | string | Expected income from swap counter party, example: "100:${SYMBOL}" or "100:${SYMBOL},10000:BTCB-1DE" The amount needs to be bumped by e^8 | No |
...
1. Swap between ${TOKENSTANDARD}2 tokens

* On *testnet*:

Command line
```shell
./eth-cli token HTLT --recipient-addr <recipient-addr> --amount 100:${SYMBOL} --expected-income <expectedIncome> --height-span <heightSpan> --from <from-addr> --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```

Javascript
```javascript
  const client = new BncClient("https://testnet-dex.${domainchain}")
  ...
  const expectedIncome = "100:${SYMBOL}"// expected income
...
2. Swap from ${Name Chain} to Ethereum

* Clients send HTLT on ${Name Chain} on *testnet*:

Command line:
```shell
./eth-cli token HTLT --from <from-addr> --chain-id ${Name-Chain}-Ganges  --height-span <heightSpan> --amount <amount> --expected-income <expectedIncome> --recipient-addr <deputy-${tokenstandard}2-addr>  --recipient-other-chain <client ethereum address>  --cross-chain --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```
...
3. Swap from Ethereum to ${Name Chain}

> Note: Once cross-chain is true, --recipient-other-chain must not be empty

* Deputy send HTLT on ${Name Chain} on *testnet*:

Command line:
```shell
./eth-cli token HTLT --from  <from-addr> --chain-id ${Name-Chain}-Ganges --height-span  <heightSpan>  --amount <amount> --expected-income <expectedIncome> --recipient-other-chain <deputy ethereum address> --sender-other-chain <client ethereum address> --recipient-addr <client ${tokenstandard}2 address> --cross-chain --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```
...
Deposit Hash Timer Locked Transfer is to lock new ${TOKENSTANDARD}2 asset to an existed HTLT which is for single chain atomic swap.
...
| Amount | Coins | The swapped out amount ${TOKENSTANDARD}2 tokens, example: "100:${SYMBOL}" or "100:${SYMBOL},10000:BTCB-1DE" | No |
...
Command line:
```shell
./eth-cli token deposit --swap-id <swapID>  --amount 10000:TEST-599 --from <from-key> --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```
...
Command line:
```shell
./eth-cli token claim --swap-id  <swapID> --random-number <random-number> --from <from-key> --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```
...
Command line:
```shell
./eth-cli token refund --swap-id <swapID> --from <from-key>  --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```
...
* On testnet:
```
./eth-cli token query-swap --swap-id <swapID> --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```
Expected output
```json
{
...
* On testnet:
```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr <address> --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```
...
* On testnet:
```
./eth-cli token query-swapIDs-by-creator  --creator-addr <address> --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```
...
-- | -- | -- | --
HTLT | N/A |  0.000375 ${SYMBOL} | Y
depositHTLT | N/A |  0.000375 ${SYMBOL} | Y
claimHTLT | N/A |  0.000375 ${SYMBOL} | Y
refundHTLT | N/A |  0.000375 ${SYMBOL} | Y
...
1. Deploy smart-contract which supports Atomic Peg Swap (APS), there is  already [one example](https://github.com/${githubusername}/${tokenstandard}3-smartcontracts) for Ethereum
2. Deploy `deputy` process for handling swap activities by token owners, there is an existing open-source solution here: <https://github.com/${githubusername}/${tokenstandard}3-deputy>
...
* Its corresponding address on testnet is: `tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr`on ${Name Chain} and `0xD93395B2771914E1679155F3EA58C41d89D96098` on Ethereum testnet

### Swap Tokens from Ethereum to ${Name Chain}
 ...
 * Prameters:
      ...
      * _${tokenstandard}2SenderAddr: omit this field with `0x0`
      * _${tokenstandard}2RecipientAddr: Decode your testnet address from bech32 encoded to hex, for example: 0xc41f2a85e1d3629637de1222017dce46c6c8e4b9
      * _outAmount:  approved amount, should be bumped by e^10
      * _${tokenstandard}2Amount: _outAmount * exchange rate, the default rate is 1
...
#### 3. Deputy Call HTLT on ${Name Chain}
Then, Deputy will send `HTLT` transaction [here](https://testnet-explorer.${domainchain}/tx/99CBC2896F0CF14DDAB0684BDA0A3E9FF2271056E68EC3559AB7FB24E0EE97DE)

#### 4. Claim HTLT on ${Name Chain}

* Get the `swapID` on ${Name Chain}

```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr tbnb1cs0j4p0p6d3fvd77zg3qzlwwgmrv3e9e63423w --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
[
  "12aacc3bdc2cef97e8e45cc9b409796df57904a4e9c76863ad8420ff75f13128"
]
```

You can also get swapID by [calculateSwapID in javascript-sdk](https://github.com/${githubusername}/javascript-sdk/blob/91b4d39e96e6433c16a3a1288931f84923075543/src/utils/index.js#L266). It requires three parameters:
...
|sender | string  | deputy ${tokenstandard}2 address | tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr |
...
* Verify parameters in the swap:
	...
	- `out_amount` should be reasonable. Please note that the decimals of ${tokenstandard}2 tokens is 8, the out_amount should be something around 10000000000:PPC, deputy will deduct some fees.
	- `expire_height` must not be passed and should be enough for send claim transaction

* Send claim transaction on ${Name Chain}

```
./eth-cli token claim --swap-id  12aacc3bdc2cef97e8e45cc9b409796df57904a4e9c76863ad8420ff75f13128  --random-number <random-number> --from <from-key>  --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```

Example of `claim` tx on [testnet](https://testnet-dex.${domainchain}/api/v1/tx/6BA714E6D107F1D9634DDC159F560A1FB61393B8E15723EFD70B9EA8B0B1AA9A?format=json)
...
#### 6. Demo for Client APP: swap erc20 to ${TOKENSTANDARD}2

This is a javascript implementation for client app to swap [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) to [PPC-00A](https://testnet-explorer.${domainchain}/asset/PPC-00A) with deputy.
...
```javascript
  ...
  const deputy${SYMBOL}WalletAddr = "tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr"
  ...
  const bnbClient = new BncClient("https://testnet-dex.${domainchain}")
  ...
  const bnbRPC = new rpcClient("https://seed-pre-s3.${domainchain}", "testnet")
  ...
  const hexEncodingClient${SYMBOL}addr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  ...
  const htltData = swapContract.methods.htlt("0x"+randomNumberHash, timestamp, heightSpan, deputyEthWalletAddr, "0x0", hexEncodingClient${SYMBOL}addr, amount, expectedIncome).encodeABI()
  ...
  //----------------------------------------------------------------------------
  //Step3 query swap created by deputy on ${Name Chain} and verify swap parameters
  //----------------------------------------------------------------------------
  const swapID = calculateSwapID(randomNumberHash.replace("0x", ""), deputy${SYMBOL}WalletAddr, clientEthWalletAddr).toString()
  console.log(swapID)
...
### Swap Tokens from ${Name Chain} to Ethereum
![image-20190918193910521](../../assets/bnc2eth.png)

#### 1. Send `HTLT` Transaction from ${Name Chain}
...
```
./eth-cli token HTLT --from atomic --recipient-addr tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr  --chain-id ${Name-Chain}-Ganges  --height-span 10000 --amount  9900000000:PPC-00A  --expected-income 9900000000:PPC  --recipient-other-chain 0x133D144F52705cEb3f5801B63b9EBcCF4102f5Ed  --cross-chain --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
...
Example is [here](https://testnet-explorer.${domainchain}/tx/9ECECE9E0F08EE78583CFA37FD4C3F03521289F0F229A612886B8B21B9C62D7F)

Then, you can query the the swap by `SwapID`:

```
./eth-cli token query-swap --swap-id f85dd907df0a5897927b949c0f9e2563d453ba698ff9941fed1ce91f8057afc2 --chain-id ${Name-Chain}-Ganges --trust-node --node http://data-seed-pre-0-s3.${domainchain}:80
```
...
- `_randomNumberHash` must equal to the randomNumberHash in client HTLT transaction on ${Name Chain}
- `_recipientAddr` must equal to client ethereum wallet address
- `_timestamp` must equal to the timestamp in client HTLT transaction on ${Name Chain}
 ...
 * Function: *claim*
 * Prameters:
    * _swapID: this is get from event, you can also calculate it from `calSwapID` function in the contract. calSwapID(randomNumberHash, {deputy ethereum address}, {hex encoding client ${name} address})
...
#### 5. Deputy Claim on ${Name Chain}

`Claim HTLT` transaction from **Deputy** is [sent](https://testnet-explorer.${domainchain}/tx/8C616DEFD2EAA41E13D2DC4844B218DFF8CFE24B4C7A693AAD700381B5FF7B48) afterwards:

#### 6. Demo for Client APP: swap ${TOKENSTANDARD}2 to erc20

This is a javascript implementation for client app to swap  [PPC-00A](https://testnet-explorer.${domainchain}/asset/PPC-00A) to [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) with deputy.
...
```javascript
  ...
  bnbClient.swap.HTLT(clientBnbWalletAddr, deputy${SYMBOL}WalletAddr, clientEthWalletAddr, "", randomNumberHash, timestamp, amount, expectedIncome, heightSpan, true)
  await wait(1000)
  ...
  //----------------------------------------------------------------------------
  const hexEncodingClient${SYMBOL}addr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  const swapID = await swapContract.methods.calSwapID("0x"+randomNumberHash, deputyEthWalletAddr, hexEncodingClient${SYMBOL}addr).call()
  console.log(swapID)
  ...
  //----------------------------------------------------------------------------
  //If step2 or step3 are failed and the expire height on ${Name Chain} is passed, try to send refundHTLT transaction on ${Name Chain}
  //----------------------------------------------------------------------------
```

### Swap between Several ${TOKENSTANDARD}2 tokens

![image-20190918193422062](../../assets/same-chain.png)

### Swap between Several ${TOKENSTANDARD}2 tokens fails
