...
State sync can help fullnode in same status with other peers within short time (according to our test, a one month ~800M DB snapshot in ${name} chain testnet can be synced in around 45 minutes). As explained in [${TOKENSTANDARD}18](https://github.com/${githubusername}/${TOKENSTANDARD}s/blob/master/${TOKENSTANDARD}18.md), State sync will get the application state of your full node to be up to date without downloading all of the blocks.The sync speed is faster than fast sync.
