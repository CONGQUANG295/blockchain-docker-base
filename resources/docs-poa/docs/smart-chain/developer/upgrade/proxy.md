# Upgradeable ${TOKENSTANDARD}20 Contracts on ${Name}
...
## Writing Upgradeable ${TOKENSTANDARD}20 Contracts
...
pragma solidity ^0.6.0;
  ...
  contract ${TOKENSTANDARD}20UpgradeableProxy is TransparentUpgradeableProxy {
...
https://github.com/${githubusername}/canonical-upgradeable-${tokenstandard}20/blob/47ed7a710e6e86bdc85f2118bf63fc892e3b7716/contracts/${TOKENSTANDARD}20TokenImplementation.sol#L37
...
```
${TOKENSTANDARD}20 contract initializes the token’s name, symbol, and decimals in its constructor. You should not use these contracts in your ${TOKENSTANDARD}20 Upgrades contract. , make sure to use the `upgradable${TOKENSTANDARD}20implementation` that has been modified to use initializers instead of constructors.
https://github.com/${githubusername}/${name}-genesis-contract/blob/42922472b43397fbca9d0c84c7f72fbfaf39efc3/contracts/${tokenstandard}20_template/${TOKENSTANDARD}20Token.template#L351
...
https://github.com/${githubusername}/canonical-upgradeable-${tokenstandard}20/blob/master/contracts/${TOKENSTANDARD}20TokenImplementation.sol

```javascript
const ${TOKENSTANDARD}20TokenImplementation = artifacts.require("${TOKENSTANDARD}20TokenImplementation");const ${TOKENSTANDARD}20TokenFactory = artifacts.require("${TOKENSTANDARD}20TokenFactory");
const Web3 = require('web3');const web3 = new Web3(new Web3.providers.HttpProvider('http://localhost:8545'));
const fs = require('fs');
module.exports = function(deployer, network, accounts) { deployer.then(async () => {  await deployer.deploy(${TOKENSTANDARD}20TokenImplementation);  await deployer.deploy(${TOKENSTANDARD}20TokenFactory, ${TOKENSTANDARD}20TokenImplementation.address); });};
```
...
```javascript
contract('Upgradeable ${TOKENSTANDARD}20 token', (accounts) => {  it('Create Token', async () => {    const ${TOKENSTANDARD}20TokenFactoryInstance = await ${TOKENSTANDARD}20TokenFactory.deployed();    ${tokenstandard}20FactoryOwner = accounts[0];    ${tokenstandard}20Owner = accounts[1];    proxyAdmin = accounts[0];
    const tx = await ${TOKENSTANDARD}20TokenFactoryInstance.create${TOKENSTANDARD}20Token("ABC Token", "ABC", 18, web3.utils.toBN(1e18), true, ${tokenstandard}20Owner, proxyAdmin, {from: ${tokenstandard}20FactoryOwner});    truffleAssert.eventEmitted(tx, "TokenCreated",(ev) => {      ${tokenstandard}20TokenAddress = ev.token;      return true;    });
  });
```
### Transfer Control
You can change the proxy owner to another address.
```js
let event = await ${tokenstandard}20proxy.methods.changeAdmin(newAdmin).send({from: proxyAdmin});
${tokenstandard}20proxy.getPastEvents("AdminChanged", {fromBlock: 0, toBlock: "latest"}).then(console.log)

```
### Transfer Owner
You can change the ${TOKENSTANDARD}20 token owner to another address.
```js
    await ${tokenstandard}20.methods.transferOwnership(accounts[5]).send({from: accounts[1]});
    const owner = await ${tokenstandard}20.methods.getOwner().call({from: accounts[5]});
```
### Deploy on Testnet

Create the following `2_${tokenstandard}20.js` script in the migrations directory.
```js
module.exports = function(deployer, network, accounts) { deployer.then(async () => {  await deployer.deploy(${TOKENSTANDARD}20TokenImplementation);  await deployer.deploy(${TOKENSTANDARD}20TokenFactory, ${TOKENSTANDARD}20TokenImplementation.address); });};
```
...
```shell
truffle(ganache)> ${TOKENSTANDARD}20TokenFactoryInstance = await ${TOKENSTANDARD}20TokenFactory.deployed();undefinedtruffle(ganache)> await ${TOKENSTANDARD}20TokenFactoryInstance.create${TOKENSTANDARD}20Token("ABC Token", "ABC", 18, web3.utils.toBN(1e18), true, {address1}, {address2});
```
> Note: any secrets such as mnemonics or ${Name} explorer keys should not be committed to version control.

Run `truffle migrate` with the ${Name} testnet to deploy.  We can see our implementation contract '${TOKENSTANDARD}20TokenImplementation' and the '${TOKENSTANDARD}20TokenFactory' being deployed.
```
Deploying '${TOKENSTANDARD}20TokenImplementation'
   ------------------------------------
   ...
   Deploying '${TOKENSTANDARD}20TokenFactory'
   -----------------------------
...
Create the new implementation, `${TOKENSTANDARD}20_V2.sol` in your contracts directory with the following Solidity code.
```js
...
```js
let tx = await ${tokenstandard}20proxy.methods.upgradeTo(newInstance.address).send({from: proxyAdmin});
 ${tokenstandard}20proxy.getPastEvents("Upgraded", {fromBlock: 0, toBlock: "latest"}).then(console.log)
```
